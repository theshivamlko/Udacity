...
/* --------------------------------------------------------------------------------- state display */
		    case 'state':
				...
				if(isset($uri_array[4]) && $uri_array[4] =="city") {
					...
					if(isset($uri_array[6])) {
						if ($uri_array[6]=="update") {
							$inUpdateCity = true;
							$isAllowedUpdate = false;
							$temCity = $db->getCity($city_id);
							$temCity = $temCity[1];

							if ($user_id == $temCity['user_id']) {
								$isAllowedUpdate = true;
							} else {
								$isAllowedError = "Only User Addedthis city can change city name.";
							}
								...
								<div class="a-ll-v form-style" id="side-1">
										 <div class="a-ll-v">
											...
											if(!$isCity) {
												...
												if($isAllowedUpdate){
													?>
													<legend><span class="number">1</span>Update City name</legend>
	   	   											 <form action="" method="post" class="feed-form-city-update" name="feed-form-city-update">
	   	   							 					<fieldset>
	   														<label for="form-city-name-update">City Name</label>
	   	   													<input type="text" name="form-city-name-update" id="form-city-name" value="<? echo $city_name;?>" required>
	   	   													<input type="hidden" name="form-city-state" id="form-city-state" value="<?php echo $state; ?>" required>
	   	   												</fieldset>
	   	   												<input type="submit" name="feed-form-city-update" value="Update" />
	   	   											</form>
													<?
												} else {
													echo $isAllowedError;
